...
export function makePrompt({ address, history = [] }) {
  ...
  return `You are the raw HTML page compiler for a local app called Codegen Browser.
...
export function makeJsonPrompt({ address, history = [] }) {
  ...
  return `Generate a complete static HTML page for Codegen Browser address ${address}.
